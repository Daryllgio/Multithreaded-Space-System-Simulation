...
void system_run(System *system) {
    ...
    if (system->amount_stored == 0) {
    	sem_wait(&system->consumed.resource->semaphore);
		result_status = system_convert(system);
		sem_post(&system->consumed.resource->semaphore);
    ...
    if (system->amount_stored  > 0) {
    	sem_wait(&system->produced.resource->semaphore);
		result_status = system_store_resources(system);
		sem_post(&system->produced.resource->semaphore);
...
void system_array_add(SystemArray *array, System *system) {
	...
	if (array->size >= array->capacity){
		System **new_array = (System**)malloc(sizeof(System*) * array->capacity * 2);
		if (!new_array) return;
