...
int main(void) {
    Manager manager;
    pthread_t manager_thread, system_threads[10];
    ...
    pthread_create(&manager_thread, NULL, (void *(*)(void *))manager_run, &manager);

    
    for (int i = 0; i < manager.system_array.size; ++i){
    	pthread_create(&system_threads[i], NULL, (void *(*)(void *))system_run, manager.system_array.systems[i]);
